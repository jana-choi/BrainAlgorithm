...
void GetPostfix(char* InfixExpression, char* PostfixExpression)
{
	...
	while(Position < Length)
	{
		...
		if(Type == OPERAND)
		{
			strcat_s(PostfixExpression, sizeof(char)*100, Token);
			strcat_s(PostfixExpression, sizeof(char)*100, " ");
		}
		else if(Type == RIGHT_PARENTHESIS)
		{
			while(!LLS_IsEmpty(Stack))
			{
				...
				if(Popped->pData[0] == LEFT_PARENTHESIS)
				{
				...
				{
					strcat_s(PostfixExpression, sizeof(char)*100, Popped->pData);
					LLS_DestroyNode(Popped);
				}
			}
		}
		...
		{
			while(!LLS_IsEmpty(Stack) && !IsPrior(LLS_Top(Stack)->pData[0], Token[0]))
			{
				Node* Popped = LLS_Pop(Stack);
				if(Popped->pData[0] != LEFT_PARENTHESIS)
					strcat_s(PostfixExpression, sizeof(char)*100, Popped->pData);
			...
			LLS_Push(Stack, LLS_CreasteNode(Token));
		}
	}
...
double Calculate(char* PostfixExpression)
{
	...
	while(Read < Length)
	{
		...
		if(Type == OPERAND)
		{
			Node* NewNode = LLS_CreasteNode(Token);
		...
		else
		{
			...
			Operator2 = atof(OperatorNode->pData);
			LLS_DestroyNode(OperatorNode);

			OperatorNode = LLS_Pop(Stack);
			Operator1 = atof(OperatorNode->pData);
			LLS_DestroyNode(OperatorNode);
			...
			LLS_Push(Stack, LLS_CreasteNode(ResultString));
		}
	}

	ResultNode = LLS_Pop(Stack);
	Result = atof(ResultNode->pData);
