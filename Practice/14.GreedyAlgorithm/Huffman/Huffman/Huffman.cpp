...
void Huffman_AddBit(BitBuffer* Buffer, char Bit)
{
	...
	if(Buffer->Size % 8 == 0)
	{
		Buffer->Buffer = (UCHAR*)realloc(Buffer->Buffer, sizeof(UCHAR) * ((Buffer->Size/8)+1));
...
void Huffman_Encode(HuffmanNode** Tree, /*UCHAR*/char* Source, BitBuffer* Encoded, HuffmanCode CodeTable[MAX_CHAR])
{
	int i = 0, j = 0;
...
void Huffman_Decode(HuffmanNode* Tree, BitBuffer* Encoded, /*UCHAR*/char* Decoded)
{
	int i = 0;
	int Index = 0;
